...
**消费链路**：
1. **Agent 路径（两轮分析）**：日线 SEPA 分析完成后，若结论为买入/强烈买入，自动启动第二轮 `run_once()` 调用。第二轮 prompt 包含最近10根60分钟K线 + RSI(14) + MA20 + 量趋势 + K线形态，用于精修入场时机（立即入场 / 等待回踩MA20 / 放弃）。
2. **Classic 路径（单轮增强）**：`format_analysis_prompt()` 在 SEPA 数据段落之后注入60分钟数据表格，LLM 在单轮内同时完成日线分析 + 60分钟精修。
3. **指标计算**：RSI(14) 和 MA20 基于 `stock_minutely` 的 close 序列在 Pipeline 中实时计算（pandas 滚动窗口），不预计算落表。
